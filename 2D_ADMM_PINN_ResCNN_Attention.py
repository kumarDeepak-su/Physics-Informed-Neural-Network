...
def load_compatible_checkpoint(net: nn.Module, checkpoint_path: str) -> None:
    ...
    if not os.path.exists(checkpoint_path):
        ...
        return
    # Project checkpoints include NumPy metadata, so PyTorch >=2.6 needs
    # weights_only=False for this loader.
...
def _tv1d_columns(A, lam, beta=1.0, n_iter=60, tol=1e-7):
    """
    Anisotropic 1-D total-variation denoising along axis 0, applied to every
    column of A simultaneously:

        argmin_V  0.5||V - A||_F^2 + lam * sum_t sum_i |V[i+1,t] - V[i,t]|.

    Solved by a small ADMM whose V-update uses a single shared Cholesky factor
    of (I + beta * Dx^T Dx) (a fixed SPD tridiagonal matrix), so all columns
    are denoised with vectorised back-substitution.  Exact at convergence.
    """
    nx, m = A.shape
    if nx < 2 or lam <= 0.0:
        return A.copy()

    # fixed tridiagonal system  M = I + beta * Dx^T Dx
    main = np.full(nx, 1.0 + 2.0 * beta)
    main[0] = main[-1] = 1.0 + beta
    off = np.full(nx - 1, -beta)
    M = np.diag(main) + np.diag(off, 1) + np.diag(off, -1)
    c_fac = cho_factor(M, lower=True)

    def Dx(X):
        return X[1:, :] - X[:-1, :]

    def DxT(Q):
        out = np.zeros((nx, Q.shape[1]))
        out[:-1, :] -= Q
        out[1:, :]  += Q
        return out
    ...
    for _ in range(n_iter):
        rhs = A + beta * DxT(Z - Ud)
        V = cho_solve(c_fac, rhs)
        DV = Dx(V)
        Z_new = soft_threshold(DV + Ud, thr)
        Ud += DV - Z_new
        if np.linalg.norm(Z_new - Z) / (np.linalg.norm(Z) + 1e-12) < tol:
            Z = Z_new
            break
        Z = Z_new
    return V
...
def admm_l1_2d_tv(S, w, L0, mu_t, alpha, lambda_t,
                  mu_x=None, lambda_x=None,
                  epsilon=1e-4, max_iter=40, tol=1e-5,
                  rho=None, vert_inner=12, reweight=True,
                  cg_iter=None, cg_tol=None):
    """
    Spatially-coupled 2-D reweighted-L1 + lateral total-variation inversion.

    This is the *fair* classical counterpart to the trace-wise He et al. solver
    (`admm_l1_single_trace`): it inverts the whole section jointly and adds an
    explicit lateral coupling term, so any advantage the physics-informed
    network shows cannot be dismissed as "merely 2-D spatial regularisation".

    Objective (L = log-impedance, shape (nx, nt)):

        min_L  1/2 || S - L @ WDh^T ||_F^2
               + alpha/2 || L - L0 ||_F^2
               + mu_t  sum_i || M_i  (D_half L_i) ||_1     (vertical RW-L1)
               + mu_x  || D_x L ||_1                        (lateral TV)

    where D_half = 0.5 D is the SAME linearised Born operator as the 1-D code
    and D_x is the first difference across traces.  The problem is solved by a
    consensus ADMM that splits the two regularisers:

        * vertical sub-problem  -> per-trace reweighted-L1 ADMM solved EXACTLY
          (identical update to `admm_l1_single_trace`, vectorised over traces);
        * lateral sub-problem   -> exact 1-D total-variation denoising
          (Condat) applied independently to every time sample.

    Because each sub-problem is solved exactly, the method is genuinely
    converged (no inner-CG conditioning issues) and reduces to the trace-wise
    He et al. result as mu_x -> 0.

    Parameters
    ----------
    mu_t      : vertical L1 weight   (== `mu` in the 1-D solver)
    alpha     : Tikhonov proximity weight to L0
    lambda_t  : vertical ADMM penalty
    mu_x      : lateral TV weight    (default 0.5*mu_t-scaled, see below)
    rho       : consensus penalty coupling the two splits (default 4*alpha)
    vert_inner: inner reweighted-L1 ADMM sweeps per outer consensus iteration
    reweight  : reweighted-L1 (True) or plain L1 (False) on the vertical term

    Returns
    -------
    Z_inv : inverted impedance = exp(L), shape (nx, nt)
    convergence : list of consensus residuals ||L - V|| / ||L|| per outer iter
    """
    nx, m = L0.shape
    nw    = len(w)
    if mu_x is None:
        mu_x = 8.0 * mu_t            # lateral TV scaled to log-impedance units
    if rho is None:
        rho = 4.0 * alpha

    # --- fixed vertical operators (identical to the trace-wise solver) -------
    D       = difference_matrix(m)            # (m-1, m)
    D_half  = 0.5 * D                         # linearised Born
    W_mat   = convolution_matrix(w, m - 1)    # (m+nw-2, m-1)
    WDh     = W_mat @ D_half                  # (m+nw-2, m)
    WDhtWDh = WDh.T @ WDh                      # (m, m)
    WDhtS   = S @ WDh                          # (nx, m)
    DhT     = D_half.T                         # (m, m-1)

    alpha_r = alpha + rho
    base    = WDhtWDh + alpha_r * np.eye(m)    # shared dense part of A_i

    # tridiagonal-band assembly for lambda_t * D_half^T diag(Mt2) D_half
    diag_idx = np.arange(m)
    off_idx  = np.arange(m - 1)

    def build_A_stack(Mt2):
        """Per-trace system matrices A_i, shape (nx, m, m)."""
        A = np.broadcast_to(base, (nx, m, m)).copy()
        # diagonal contribution 0.25*(p[k-1]+p[k]) ; off-diag -0.25*p[k]
        d = np.zeros((nx, m))
        d[:, :-1] += 0.25 * Mt2
        d[:, 1:]  += 0.25 * Mt2
        A[:, diag_idx, diag_idx] += lambda_t * d
        offd = -0.25 * lambda_t * Mt2
        A[:, off_idx, off_idx + 1] += offd
        A[:, off_idx + 1, off_idx] += offd
        return A

    # --- consensus / split variables -----------------------------------------
    L  = L0.copy()
    V  = L0.copy()
    U  = np.zeros((nx, m))

    # vertical ADMM state (warm-started across outer iterations)
    Rv = np.zeros((nx, m - 1))
    Cv = np.zeros((nx, m - 1))
    Mt = np.ones((nx, m - 1))
    ...
    for outer in range(max_iter):
        ...
        # consensus target folded into a single Tikhonov term
        T = (alpha * L0 + rho * (V - U)) / alpha_r        # (nx, m)
        WDhtS_T = WDhtS + alpha_r * T                     # RHS data+prox part

        # ----- vertical sub-problem : per-trace reweighted-L1 (vectorised) ---
        for _ in range(vert_inner):
            A_stack = build_A_stack(Mt ** 2)
            b = WDhtS_T + lambda_t * ((Mt * (Rv - Cv)) @ D_half)   # (nx, m)
            L = np.linalg.solve(A_stack, b[:, :, None])[:, :, 0]
            y = L @ DhT                                    # reflectivity (nx,m-1)
            Rv = soft_threshold(Mt * y + Cv, mu_t / lambda_t)
            Cv = Cv + (Mt * y - Rv)
            if reweight:
                Mt = 1.0 / (np.abs(y) + epsilon)

        # ----- lateral sub-problem : 1-D TV denoise over traces (all columns) -
        A_lat = L + U                                      # (nx, m)
        if mu_x > 0.0:
            V = _tv1d_columns(A_lat, lat_lam)
        else:
            V = A_lat.copy()
        ...
        rel = np.linalg.norm(L - L_old) / (np.linalg.norm(L_old) + 1e-12)
        convergence.append(rel)
        if rel < tol:
            break

    # consensus estimate (L and V agree at convergence)
    return np.exp(0.5 * (L + V)), convergence
