...
==============================================================================
  ...
  Author : Dr. Deepak Kumar, IG PAN, Warsaw
  Date   : 2026-03-18
==============================================================================
"""
...
# ── 2D model geometry ────────────────────────────────────────────────────────
NX         = 400       # laptop-sized 2D run: number of traces (lateral)
NT         = 300       # laptop-sized 2D run: time samples per trace
...
EPOCHS      = 80        # laptop run; increase to 300+ for final paper figures
BATCH_SIZE  = 8         # smaller CPU batches for laptop runs
LR          = 5e-4      # initial learning rate
LR_MIN      = 1e-6      # cosine annealing minimum LR
WARMUP_EPOCHS = 25      # epochs to anneal w_init for laptop run
...
def refresh_output_paths() -> None:
    global RESULTS_NPZ, UNET_CLEAN_CKPT, UNET_NOISY_CKPT
...
def parse_args():
    ...
    parser.add_argument("--preset", choices=["laptop", "server"], default="laptop",
                        help="Use laptop defaults or larger server/HPC defaults.")
...
def configure_runtime(args) -> None:
    ...
    global NOISE_SNR, RESULTS_DIR, RUN_ATTENTION, RUN_TAG
    ...
    elif args.preset == "server" or args.run_tag is not None:
...
def train_model_2d(S_patches, L_init_patches, wavelet,
    ...
    for epoch in range(EPOCHS):
        ...
        # Anneal initial-model proximity weight
...
if __name__ == "__main__":
    ...
    time_ms = np.arange(NT) * DT * 1000

    wavelet = ricker_wavelet(F0, DT)
    nw = len(wavelet)
    ...
    result_payload = dict(
             ...
             model_type='complex_fault_oval_body',
             preset=args.preset,
             run_tag=RUN_TAG,
